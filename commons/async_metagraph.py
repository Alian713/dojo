...
from bittensor.metagraph import get_save_dir, latest_block_path, METAGRAPH_STATE_DICT_NDARRAY_KEYS
...
class MetagraphMixin(ABC):
    ...
    def _set_weights_and_bonds(self, subtensor: Optional[bt.subtensor] = None):
        """
        ...
        for item in data:
            if len(item) == 0:
                ...
                if attribute == "weights":
                    data_array.append(
                        bt.utils.weight_utils.convert_weight_uids_and_vals_to_tensor(
    ...
    def _process_root_weights(
        self, data, attribute: str, subtensor: bt.subtensor
    ) -> Union[NDArray, "torch.nn.Parameter"]:
        """
...
class AsyncMetagraph(MetagraphMixin):
    def __init__(
        ...
        self.version = (np.array([bt.__version_as_int__], dtype=np.int64),)
    ...
    def _set_metagraph_attributes(self, block, subtensor):
        ...
        self.version = self._create_tensor(
            [bt.__version_as_int__], dtype=np.int64
        )
    ...
    def load_from_path(self, dir_path: str) -> "metagraph":  # type: ignore
        ...
        try:
            ...
            try:
                ...
                bt.__console__.print("Unable to load file. It may be corrupted.")
                raise
...
def metagraph(
    ...
    metagraph_ = AsyncMetagraph(
        network = network, netuid = netuid, lite = lite, sync = False
    )
    metagraph_.sync(block = block, lite = lite)
